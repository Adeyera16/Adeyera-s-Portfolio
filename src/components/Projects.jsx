         ...
         <div className='grid md:grid-cols-3 gap-12 mx-[5rem] mt-[3rem] text-gray-500'>
            {project.map((item) => (
              <div key={item.id} className='h-auto w-full group bg-white shadow-xl shadow-gray-300 p-5 '>
                ...
                <div className='my-4'>
                <h3 className='text-xl font-bold tracking-wider text-black my-4'>{item.title}</h3>
                  <p className='my-4'>{item.description}</p>
                  <div className='flex justify-between'>
                    <Link to={item.demoLink} target='_blank' rel='noopener noreferrer' className='mt-2 p-2 bg-black text-white rounded-md '>
                    Live Demo
                    </Link>
